...
#include "arvces_gigeBasler_gige_basler.h"
...
BOOL ImageLiveStartBasler(uint32_t nNoOfImages = 50000000, int nArrMode = 0);//0 = char array and 1 = int array for image transfer);
...
Pylon::CBaslerGigEInstantCamera m_CameraBasler;
...
int * Image;
int * defImage;//to be sent when no image in que
...
BOOL InitCameraBasler(long lCameraSerial)
{
	...
	try
	{
		...
		if (!m_CameraBasler.IsGigE())
		{
			m_CameraBasler.DetachDevice();
			printf("GIGE cam Not Found\n");
		}
		else
			printf("GIGE cam Found\n");
		
		m_CameraBasler.Open();
		printf(" GIGE InitCameraBasler->Open\n");

		m_strCamera1Serial = m_CameraBasler.GetDeviceInfo().GetSerialNumber();
		printf(" GIGE CAM Serial No : %s\n", m_strCamera1Serial);
		...
		if (m_bValidCam)
		{
			...
			printf(" GIGE Trying to set exposure\n");
			SetExposureBasler(fExposureValue);
			printf(" GIGE Exposure Set\n");

			//set Max AOI
			printf("GIGE Setting AOI\n");
			SetCameraAOI(0,0,100,100);

			printf("GIGE Setting Trigger Mode\n");
...
BOOL SetExposureBasler(float fExpTimeMS, float fGain, float fBrightness, float fContrast)
{
	...
	try
	{
		if (m_bValidCam)
		{
			...
			printf("Trying to set Exposure\n");
			// Set Exposure
			m_CameraBasler.AcquisitionMode.SetValue(Basler_GigECameraParams::AcquisitionMode_Continuous);
			m_CameraBasler.ExposureAuto.SetValue(Basler_GigECameraParams::ExposureAuto_Off);
			m_CameraBasler.ExposureTimeAbs.SetValue(fExpTimeMS);
			double e = m_CameraBasler.ExposureTimeAbs.GetValue();
			printf("Setting Exposure: %3.3f \n", e);

			//Set Gain
			m_CameraBasler.GainAuto.SetValue(Basler_GigECameraParams::GainAuto_Off);
			m_CameraBasler.GainRaw.SetValue(m_CameraBasler.GainRaw.GetMin());
			e = m_CameraBasler.GainRaw.GetValue();
			printf("Setting Gain: %3.3f \n", e);

			//Set Pixel format
			m_CameraBasler.PixelFormat.SetValue(Basler_GigECameraParams::PixelFormat_Mono8);
	...
	catch (GenICam::GenericException &e)
	{
		...
		//CString strError = e.GetDescription();
		//AfxMessageBox(strError);
		printf("GIGE-SetExposureBasler Exception Caught \n");
		if (m_CameraBasler.IsOpen())
			m_CameraBasler.Close();
		if (m_CameraBasler.IsPylonDeviceAttached())
			m_CameraBasler.DetachDevice();
...
BOOL SetCameraAOI(int nOffsetX, int nOffsetY, int nWidth, int nHeight)
{
	...
	try
	{
		if (m_bValidCam)
		{
			...
			defImage = (int*)calloc(sizeof(int), 1);
			if (!m_CameraBasler.IsPylonDeviceAttached())
				m_CameraBasler.Attach(CTlFactory::GetInstance().CreateFirstDevice());
			if (!m_CameraBasler.IsOpen())
				m_CameraBasler.Open();
	...
	catch (GenICam::GenericException &e)
	{
		...
		//CString strError = e.GetDescription();
		//AfxMessageBox(strError);
		printf("GIGE-SetCameraAOI Exception Caught \n");
		if (m_CameraBasler.IsOpen())
			m_CameraBasler.Close();
		if (m_CameraBasler.IsPylonDeviceAttached())
			m_CameraBasler.DetachDevice();
...
BOOL SetCameraTriggerMode(int nTriggerMode, int nTriggerSource, int nTriggerActivation)
{
	try
	{
		if (m_bValidCam)
		{
			...
			m_CameraBasler.TriggerMode.SetValue((Basler_GigECameraParams::TriggerModeEnums)nTriggerMode);
			int nTriggerAMode = m_CameraBasler.TriggerMode.GetValue();
			printf("Trigger Mode Set to : %d \n", nTriggerAMode);

			m_CameraBasler.AcquisitionMode.SetValue(Basler_GigECameraParams::AcquisitionMode_SingleFrame);
			int nAcMode = (int)m_CameraBasler.AcquisitionMode.GetValue();
			printf("Acquisition Mode Set to : %d \n", nAcMode);

			m_CameraBasler.TriggerSource.SetValue((Basler_GigECameraParams::TriggerSourceEnums)nTriggerSource);
			...
			m_CameraBasler.TriggerActivation.SetValue((Basler_GigECameraParams::TriggerActivationEnums)nTriggerActivation);
	...
	catch (GenICam::GenericException &e)
	{
		...
		//CString strError = e.GetDescription();
		//AfxMessageBox(strError);
		printf("GIGE-SetCameraTriggerMode Exception Caught \n");
		if (m_CameraBasler.IsOpen())
			m_CameraBasler.Close();
		if (m_CameraBasler.IsPylonDeviceAttached())
			m_CameraBasler.DetachDevice();
...
BOOL SetImageOrientation(int nOrientationMode)
{
	...
	catch (GenICam::GenericException &e)
	{
		...
		//CString strError = e.GetDescription();
		//AfxMessageBox(strError);
		printf("GIGE-SetImageOrientation Exception Caught \n");
		if (m_CameraBasler.IsOpen())
			m_CameraBasler.Close();
		if (m_CameraBasler.IsPylonDeviceAttached())
			m_CameraBasler.DetachDevice();
...
BOOL GrabCameraFrame(int nArrMode)//0 = char array and 1 = int array for image transfer
{
	if (m_bValidCam)
	{
...
BOOL ImageLiveStartBasler(uint32_t nNoOfImages, int nArrMode)
{
	try
	{
		...
		while (m_CameraBasler.IsGrabbing() && m_bLive)
		{
			...
			if (ptrGrabResult->GrabSucceeded())
			{
				...
				printf("Grab # : %ld ",lImageNum);
				lImageNum++;
				u_char *pImageBuffer = (u_char *)ptrGrabResult->GetBuffer();
				...
				if (pImageBuffer != NULL)
				{
					...
					{
						m_dequeIntImage_Data.push_back(arrIntImage_data);
	...
	catch (GenICam::GenericException &e)
	{
		...
		//CString strError = e.GetDescription();
		//AfxMessageBox(strError);
		printf("GIGE-ImageLiveStartBasler Exception Caught \n");
		if (m_CameraBasler.IsOpen())
			m_CameraBasler.Close();
		if (m_CameraBasler.IsPylonDeviceAttached())
			m_CameraBasler.DetachDevice();
...
JNIEXPORT jint JNICALL Java_arvces_gigeBasler_gige_1basler_InitCamera(JNIEnv *env, jclass type, jlong lCamSerial)
{
...
JNIEXPORT void JNICALL Java_arvces_gigeBasler_gige_1basler_DeInitCamera(JNIEnv *env, jclass type)
{
	DeInitCameraBasler();
}

JNIEXPORT void JNICALL Java_arvces_gigeBasler_gige_1basler_SetExposure(JNIEnv *env, jclass type, jfloat fexposure)
{
...
JNIEXPORT void JNICALL Java_arvces_gigeBasler_gige_1basler_SetAOI(JNIEnv *env, jclass type, jint offX, jint offY, jint width, jint height)
{
	SetCameraAOI((int)offX, (int)offY, (int)width, (int)height);
}

JNIEXPORT jcharArray JNICALL Java_arvces_gigeBasler_gige_1basler_GetBaslerImageinArr(JNIEnv * env, jclass type, jboolean bSaveImageOpencv)
{
#ifdef USE_OPENCV
...
JNIEXPORT jintArray JNICALL Java_arvces_gigeBasler_gige_1basler_GetBaslerImageinArrIntArr(JNIEnv * env, jclass type, jboolean bSaveImageOpencv)
{
#ifdef USE_OPENCV
	bUseOpencvSaveImage = bSaveImageOpencv;
#endif
	...
	jintArray jretarrDef = env->NewIntArray(1);
//	GrabCameraFrame(5);
	//int * Image; //making it global
	//Image = (int*)calloc(sizeof(int), m_nWidth * m_nHeight);
	if (TryEnterCriticalSection(&g_cs1))
	{
		...
		else
		{
			...
			env->SetIntArrayRegion(jretarrDef, 0, nImageWidth * nImageHeight, (jint*)defImage);
			printf("Size = 0 , Default Image Set /n");

			return jretarrDef;//return default
		}
		LeaveCriticalSection(&g_cs1);
	}
	else
...
JNIEXPORT void JNICALL Java_arvces_gigeBasler_gige_1basler_SetTriggerMode(JNIEnv *env, jclass type, jint nTriggerMode, jint nTriggerSource, jint nTriggerActivation)
{
	int o = nTriggerMode;
	...
	printf("Trigger Modes Set... \n");
}

JNIEXPORT void JNICALL Java_arvces_gigeBasler_gige_1basler_SetOrientation(JNIEnv *env, jclass type, jint nOrientMode)//currently using it for starting thread and waiting for ending its operation
{
	int n = nOrientMode;
